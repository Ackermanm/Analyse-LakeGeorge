...
def read_dataset(filepath):
    """
        ...
        Returns
        -------
        List of string list. The data set.
...
def get_column(data, header):
    """
        Get the data under a specific header from the data set.

        Parameters
        ----------
        data: List. The whole data set.
        ...
        """
    headers = data[0]
    flag = 0  # A flag which is used to get the index of the header.
...
def largest_area(data):
    """
        Get the largest area in the data.
...
def calculate_average(data, header):
...
def average_volume(data):
    """
        Get the average volume in the data.
...
def most_average_rainfall(data):
    """
       Get the time whose rainfall is closest to the average.

       Returns
       -------
       List. Contains the month and year, and the rainfall during the month.
...
def hottest_month(data):
    """
        Determine witch month is the hottest month.

        Returns
        -------
        List. The format is [month(string), average max temperature of the month(float)].
...
def area_vs_volume(data):
    """
        Get a plot of area against volume.
...
def lake_george_complex_model(data):
    """
        ...
        evaporation = −3*min_temperature + 1.6*max_temperature − 2.5*wind_speed + 4.5*solar_exposures − 0.4*humidity.
